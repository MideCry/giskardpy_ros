...
class GenericWorldConfig(WorldConfig):
    ...
    def get_tf_root_that_is_not_in_world(self) -> str:
        ...
        return self.world.groups[
            list(self.world.group_names)[0]
        ].root_link_name.short_name
    ...
    def has_odom(self) -> Optional[Tuple[str, str]]:
        try:
            ...
            message: Odometry = wait_for_message(
                Odometry, rospy.node, topic_name, qos_profile=10
            )[1]
...
class GenericCollisionAvoidance(CollisionAvoidanceConfig):
    ...
    def setup(self):
        ...
        self.set_default_external_collision_avoidance(
            soft_threshold=0.1, hard_threshold=0.0
        )
        ...
        for joint_name in ["r_elbow_flex_joint", "l_elbow_flex_joint"]:
            self.overwrite_external_collision_avoidance(
                joint_name, soft_threshold=0.05, hard_threshold=0.0
            )
        for joint_name in ["r_forearm_roll_joint", "l_forearm_roll_joint"]:
            self.overwrite_external_collision_avoidance(
                joint_name, soft_threshold=0.025, hard_threshold=0.0
            )
        self.fix_joints_for_collision_avoidance(
            ["r_gripper_l_finger_joint", "l_gripper_l_finger_joint"]
        )
        self.overwrite_external_collision_avoidance(
