...
class QPDataPublisherConfig:
    ...
    def any(self) -> bool:
        return any([self.publish_lb, self.publish_ub, self.publish_lbA, self.publish_ubA, self.publish_bE,
                    self.publish_Ax, self.publish_Ex, self.publish_xdot, self.publish_weights, self.publish_g,
                    self.publish_debug])


class PublishDebugExpressions(GiskardBehavior):
    
    def __init__(self, publish_config: QPDataPublisherConfig, name: str = 'publish qp data'):
        super().__init__(name)
