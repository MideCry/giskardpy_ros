...
class CompileMonitors(GiskardBehavior):
    ...
    def add_payload_monitors_to_behavior_tree(
        self, traj_tracking: bool = False
    ) -> None:
        payload_monitors = sorted(
            god_map.monitor_manager.payload_monitors,
            key=lambda x: isinstance(x, CancelMotion),
        )
        for monitor in payload_monitors:
            if traj_tracking:
                GiskardBlackboard().tree.execute_traj.base_closed_loop.check_monitors.add_monitor(
                    monitor
                )
            else:
                GiskardBlackboard().tree.control_loop_branch.check_monitors.add_monitor(
                    monitor
                )
