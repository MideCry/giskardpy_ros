...
class ExecutionStateToDotParser:
    ...
    def add_node(
        ...
        label = self.format_motion_graph_node_msg(
            msg=node_msg, obs_state=obs_state, life_cycle_state=life_cycle_state
        )
        node = pydot.Node(
            node_id,
            label=label,
        ...
        self.add_boarder_to_node(
            graph=graph,
            node=node,
            num=num_extra_boarders,
            color="black",
            style=boarder_style,
        )
    ...
    def add_goal_cluster(
        ...
        for i, task in enumerate(self.execution_state.tasks):
            ...
            if self.execution_state.task_parents[i] == self.cluster_name_to_goal_name(
        ...
        for i, monitor in enumerate(self.execution_state.monitors):
            if self.execution_state.monitor_parents[
                i
            ] == self.cluster_name_to_goal_name(parent_cluster.get_name()):
